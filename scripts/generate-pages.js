...
export async function generateGamePages(){
  ...
  for(const item of index.games.filter(game=>game.status==="active")){
    ...
    for(const locale of ["en","pt-BR"]){
      ...
      const seoDescription=locale==="pt-BR"?`Confira códigos ativos de ${title}, veja como resgatá-los e encontre dicas úteis para jogar.`:`Find active ${title} codes, learn how to redeem them, and get helpful gameplay tips.`;
      const ogDescription=locale==="pt-BR"?`Códigos ativos, instruções de resgate e dicas para ${title}.`:`Active ${title} codes, redemption steps, and gameplay tips.`;
      const values={
